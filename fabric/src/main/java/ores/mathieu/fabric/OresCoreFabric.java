...
//
// Description: Main entry point for the Fabric loader. Initializes common logic, registers server lifecycle events for resource reloads, and hooks into the data pack reloading system.
//
// Author: __mathieu
// Version: 26.1.100
//
...
public class OresCoreFabric implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        net.fabricmc.fabric.api.event.lifecycle.v1.ServerLifecycleEvents.END_DATA_PACK_RELOAD.register((server, resourceManager, success) -> {
            if (success) {
                ores.mathieu.OresCoreCommon.LOGGER.info("[ORES CORE] Reloading ore generation configurations from /reload...");
                ores.mathieu.config.ConfigManager.loadVeins();
            }
        });
    }
}
