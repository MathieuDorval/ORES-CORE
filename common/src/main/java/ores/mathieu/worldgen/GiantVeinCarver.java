...
//
// Description: Advanced world carver implementation for GIANT-type ore veins. 
// It wraps a standard CaveWorldCarver to hollow out large subterranean spaces, 
// then fills them with a probabilistic mixture of host blocks (like tuff or granite), 
// dynamic ore blocks, and rare bonus blocks (like raw ores) according to live 
// TOML configurations. Uses ThreadLocal indexing to synchronize multi-threaded 
// carving phases with the correct vein parameters.
//
// Author: __mathieu
// Version: 26.1.100
//
...
public class GiantVeinCarver extends WorldCarver<VeinCarverConfiguration> {
    ...
    public boolean carve(CarvingContext context, VeinCarverConfiguration config, ChunkAccess chunk, Function<BlockPos, Holder<Biome>> biomeAccessor, RandomSource random, Aquifer aquifer, ChunkPos centerChunkPos, CarvingMask mask) {
        ...
        for (String stoneId : ores.mathieu.registry.DiscoveryManager.REGISTRY.getStonesReplacement().keySet()) {
